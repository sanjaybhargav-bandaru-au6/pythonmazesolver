...
#    IT RETURNS FALSE(-1) IF NO PATH IS AVAILABLE,OTHERWISE RETURNS TRUE.
...
def print_solution(sol):
    f1.write("***\n" + "Following the path " + moveCheck(sol)
...
def moveCheck(solved_maze):
    ...
    while i != n-1 or j != n-1:
        if j == n-1:
            if solved_maze[i+1][j] == 1 and reached[i+1][j] is False:
                reached[i+1][j] = True
                i = i+1
                arr += "D"
            elif solved_maze[i][j-1] == 1 and reached[i][j-1] is False:
                reached[i][j-1] = True
                j = j-1
                arr += "L"
        elif i == n-1:
            if solved_maze[i][j+1] == 1 and reached[i][j+1] is False:
                reached[i][j+1] = True
                j = j+1
                arr += "R"
            elif solved_maze[i-1][j] == 1 and reached[i-1][j] is False:
                reached[i-1][j] = True
                i = i-1
                arr += "U"
        else:
